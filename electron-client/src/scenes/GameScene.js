...
class GameScene extends Phaser.Scene {
    ...
    init(data) {
        ...
        if (this.spectator) {
            this.mapCells = data.mapCells;

            for (let p of Object.keys(data.rivals)) {
                this.playerData[p] = {x: data.rivals[p].posX, y: data.rivals[p].posY};
            }
        } else {
    ...
    create() {
        ...
        if (this.spectator) {
            ...
            this.cam.setZoom(Math.min(zoomX, zoomY));
        }
        ...
        if (this.spectator) {
            GameManager.eventEmitter.on("NewTurnSpectator", data => {
